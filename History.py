...
# ── Guard: ถ้า df ว่างหรือไม่มี column ที่ต้องการ ──────────────────────────
REQUIRED = ["Timestamp", "Action", "SN", "PID", "Detail", "Performed By"]
if df.empty or not all(c in df.columns for c in REQUIRED):
    st.info("ยังไม่มี Log การเปลี่ยนแปลง")
    st.caption("💡 Log จะเริ่มบันทึกเมื่อ Admin ทำ Action ครั้งแรก (Deploy / Receive / Swap ฯลฯ)")
    st.stop()
